...
class IMG_Stegno:
    ...
    #Pic-Speak- main frame or start page
    def main(self, root):
        root.title('Pic-Speak')
        ...
        title = Label(frame,text='Pic-Speak Image Steganography')
        title.config(font=('Times new roman',25, 'bold'))
        title.grid(pady=20)
        title.config(bg = '#f0f0f0')
        title.grid(row=1)

        encode = Button(frame,text="Encode",command= lambda :self.encode_frame1(frame), padx=14,bg = '#f0f0f0' )
        encode.config(font=('Times new roman',14), bg='#98F5FF')
        encode.grid(row=2)
        decode = Button(frame, text="Decode",command=lambda :self.decode_frame1(frame), padx=14,bg = '#f0f0f0')
        decode.config(font=('Times new roman',14), bg='#98F5FF')
        decode.grid(pady = 12)
        decode.grid(row=3)

        my_img = Image.open("logo.png")
        new_image = my_img.resize((400,350))
        img = ImageTk.PhotoImage(new_image)
        board = Label(frame, image=img)
        board.image = img
        self.output_image_size = os.stat("logo.png")
    ...
    #Pic-Speak- back function to loop back to main screen
    def back(self,frame):
        frame.destroy()
        self.main(root)

    
    #Pic-Speak- frame for encode page
    def encode_frame1(self,F):
        F.destroy()
        F2 = Frame(root)
        label1= Label(F2,text='Select the Image in which \nyou want to hide text :')
        label1.config(font=('Times new roman',25, 'bold'),bg = '#f0f0f0')
        label1.grid(pady=20)

        button_bws = Button(F2,text='Select',command=lambda : self.encode_frame2(F2))
        button_bws.config(font=('Times new roman',18), bg='#98F5FF')
        button_bws.grid()
        button_back = Button(F2, text='Cancel', command=lambda : IMG_Stegno.back(self,F2))
        button_back.config(font=('Times new roman',18),bg='#98F5FF')
        button_back.grid(pady=15)
        button_back.grid()

        my_img = Image.open("logo.png")
        new_image = my_img.resize((350,300))
        img = ImageTk.PhotoImage(new_image)
    ...
    #Pic-Speak- frame for decode page
    ...
    #Pic-Speak- function to encode image 
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            encode_button.config(font=('Times new roman',14), bg='#98F5FF')
            ...
            button_back.config(font=('Times new roman',14), bg='#98F5FF')
            button_back.grid(pady=15)
            encode_button.grid()
            e_pg.grid(row=1)
            e_F2.destroy()
    ...
    #Pic-Speak- function to decode image 
    ...
    #Pic-Speak- function to decode data
    ...
    #Pic-Speak- function to generate data
    ...
    #Pic-Speak- function to modify the pixels of image
    ...
    #Pic-Speak- function to enter the data pixels in image
    ...
    #Pic-Speak- function to enter hidden text
